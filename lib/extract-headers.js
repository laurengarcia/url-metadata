const INCLUDED_HEADERS = new Set([
  ...
  // ScraperAPI.com - Proxy response headers
  'sa-credit-cost',
  'sa-screenshot',
...
module.exports = function serializeHeaders (headers) {
  ...
  headers.forEach((value, key) => {
    // Prefix: Include all headers that start with `x-` or `X-`, for ex:
    //  - 'x-robots-tag',
    ...
    // Prefix: Include headers that start w 'WSA' or 'Searchdata'
    // from WebScrapingAPI.com API
    if (key.toLowerCase().slice(0,3) === "wsa") {
      serialized[key] = value;
    }
    if (key.toLowerCase().slice(0,10) === "searchdata") {
      serialized[key] = value;
    }
